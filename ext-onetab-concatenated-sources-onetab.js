...
;function b1(kXS, hXS) {
    if (zj())
        kXS.style.paddingLeft = hXS;
    else
        kXS.style.paddingRight = hXS
}
;function Cj(GXS, MXS) {
    if (zj())
        GXS.style.paddingRight = MXS;
    else
        GXS.style.paddingLeft = MXS
}
;function CX(_XS, WXS) {
    if (zj())
        _XS.style.marginLeft = WXS;
    else
        _XS.style.marginRight = WXS
}
;var K1 = [{
        ...
        if (VKS._j) {
            if (eKS != VKS.FW) {
                ...
                for (var sp in VKS.kW) {
                    var qp = VKS.kW[sp];
                    if (f1(qp.parentNode, DKS) && DKS != qp)
...
;function I(iKS) {
    if (typeof iKS == 'string')
        iKS = document.getElementById(iKS);
    if (!iKS)
        return;
    while (iKS.childNodes.length > 0)
        iKS.removeChild(iKS.childNodes[0])
}
;function P1(EKS) {
...
function nX(CtS, ItS) {
    ...
    ItS = ItS || mtS.length;
    CtS = CtS || 22;
    for (jtS = 0; jtS < CtS; jtS++)
        ptS[jtS] = mtS[0 | Math.random() * ItS];
    return ptS.join('')
}
;function Rj() {
    return nX()
...
;function M(StS) {
    ...
    for (var ctS in PtS) {
        if (PtS[ctS] == '')
            ...
            gtS.appendChild(document.createTextNode(PtS[ctS]))
    }
    return gtS
}
;function B(vtS) {
    var NtS = new Date(vtS.valueOf()), wtS = (vtS.getUTCDay() + 6) % 7, itS;
    ...
    MtS.sort(function(DM, lM) {
        if (DM['starred'] || lM['starred']) {
            if (!lM['starred'])
                ...
                if (DM['starredDate'] > lM['starredDate'])
                    return 1;
                if (DM['starredDate'] < lM['starredDate'])
                    return -1;
                return 0
            ...
            if (DM['createDate'] > lM['createDate'])
                return -1;
            if (DM['createDate'] < lM['createDate'])
                return 1;
            return 0
...
;function nj(MhS, WhS, GhS) {
    return function() {
        for (var aM in WhS)
            if (window.event.toElement == WhS[aM])
                return;
...
;function vj() {
    if (nK || mK)
        return (mK ? 0 : document.documentElement.scrollTop) + document.body.scrollTop;
    else
        return window.scrollY
}
;function sX() {
    if (nK || mK)
        return (mK ? 0 : document.documentElement.scrollLeft) + document.body.scrollLeft;
    else
...
function QX(j2S) {
    ...
    c2S.appendChild(function() {
        ...
        var MM = new $K(GM,x1('deleteAll'),30,function(yJ) {
            ...
            if (Z2S >= 2)
                aJ = x1('areYouSureYouWantToDeleteTheseTabs');
            if (confirm(aJ)) {
                V2S(j2S['id'], false);
                r2S.parentNode.removeChild(r2S)
            }
        ...
        GM.appendChild(function() {
            ...
            if (W6S)
                uJ.style['boxShadow'] = 'rgb(221, 221, 221) 1px 1px 1px 1px';
            ...
            uJ.appendChild(SJ(j2S['starred'] ? x1('unstarTabGroup') : x1('starTabGroup'), 6, function(He, Ze) {
                ...
                if (j2S['starred'])
                    j2S['starredDate'] = new Date().getTime();
        ...
        SF.appendChild(Tj(x1('shareAllAsWebPage'), A2S, function(rq) {
            ...
            e1(function(xe) {
                ...
                for (var pe = 0; pe < Ke.length; pe++) {
                    var Fe = Ke[pe];
                    if (!Fe['createDate'])
                        Fe['createDate'] = new Date().getTime()
                }
                XX(Ke);
                for (var pe = 0; pe < Ke.length; pe++) {
                    var Fe = Ke[pe];
                    for (var Ie in Fe['tabsMeta'])
                        Qq.push(Fe['tabsMeta'][Ie])
                }
                if (Qq.length == 0) {
...
;function u1(k2S) {
    ...
    for (var _2S = 0; _2S < s2S.length; _2S++) {
        var W2S = s2S[_2S];
        if (!W2S['createDate'])
            W2S['createDate'] = new Date().getTime()
    }
    var G2S = 0;
    for (var _2S = 0; _2S < s2S.length; _2S++) {
        var W2S = s2S[_2S];
