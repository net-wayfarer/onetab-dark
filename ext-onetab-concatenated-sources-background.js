...
var z = function(dn, kn, hn) {
    A1(function(KB, RB) {
        ...
        if (RB) {
            for (var fB in KB)
                if (parseInt(KB[fB]['index']) != parseInt(RB['index']))
                    TB.push(KB[fB]);
            if (TB.length > 0) {
            ...
            if (!W6S)
                alert('no active tab (B)');
            else if (D1)
                console.log('no active tab (B)');
            kn(hn)
        }
    })
};
var hK = function(Mn, Gn) {
    A1(function(IB, xB) {
        ...
        if (xB) {
            for (var pB in IB)
                if (parseInt(IB[pB]['index']) < parseInt(xB['index']))
                    FB.push(IB[pB]);
            if (FB.length > 0) {
...
var wX = function(Wn, _n) {
    A1(function(yB, mB) {
        ...
        if (mB) {
            for (var jB in yB)
                if (parseInt(yB[jB]['index']) > parseInt(mB['index']))
                    CB.push(yB[jB]);
            if (CB.length > 0) {
...
;function M(hT) {
    ...
    for (var MT in kT) {
        if (kT[MT] == '')
            ...
            GT.appendChild(document.createTextNode(kT[MT]))
    }
    return GT
}
;function B(JT) {
    var _T = new Date(JT.valueOf()), WT = (JT.getUTCDay() + 6) % 7, BQ;
    ...
    ZQ.sort(function(B6, JG) {
        if (B6['starred'] || JG['starred']) {
            if (!JG['starred'])
                ...
                if (B6['starredDate'] > JG['starredDate'])
                    return 1;
                if (B6['starredDate'] < JG['starredDate'])
                    return -1;
                return 0
            ...
            if (B6['createDate'] > JG['createDate'])
                return -1;
            if (B6['createDate'] < JG['createDate'])
                return 1;
            return 0
