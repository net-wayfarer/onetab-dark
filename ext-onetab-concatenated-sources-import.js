...
;function I(Mb) {
    ...
    if (!Mb)
        return;
    while (Mb.childNodes.length > 0)
        Mb.removeChild(Mb.childNodes[0])
}
;function P1(Gb) {
...
;function M(Of) {
    ...
    for (var df in sf) {
        if (sf[df] == '')
            ...
            hf.appendChild(document.createTextNode(sf[df]))
    }
    return hf
}
;function B(Gf) {
    var Mf = new Date(Gf.valueOf()), kf = (Gf.getUTCDay() + 6) % 7, Wf;
    ...
    QS.sort(function(K_, f_) {
        if (K_['starred'] || f_['starred']) {
            if (!f_['starred'])
                ...
                if (K_['starredDate'] > f_['starredDate'])
                    return 1;
                if (K_['starredDate'] < f_['starredDate'])
                    return -1;
                return 0
            ...
            if (K_['createDate'] > f_['createDate'])
                return -1;
            if (K_['createDate'] < f_['createDate'])
                return 1;
            return 0
...
;function nj($LS, QLS, YLS) {
    return function() {
        for (var N_ in QLS)
            if (window.event.toElement == QLS[N_])
                return;
...
;function b1(MLS, kLS) {
    if (zj())
        MLS.style.paddingLeft = kLS;
    else
        MLS.style.paddingRight = kLS
}
;function Cj(WLS, GLS) {
    if (zj())
        WLS.style.paddingRight = GLS;
    else
        WLS.style.paddingLeft = GLS
}
;function CX(JLS, _LS) {
    if (zj())
        JLS.style.marginLeft = _LS;
    else
        JLS.style.marginRight = _LS
}
;var K1 = [{
