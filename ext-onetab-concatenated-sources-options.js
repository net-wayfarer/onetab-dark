...
;function b1(IjS, pjS) {
    if (zj())
        IjS.style.paddingLeft = pjS;
    else
        IjS.style.paddingRight = pjS
}
;function Cj(CjS, mjS) {
    if (zj())
        CjS.style.paddingRight = mjS;
    else
        CjS.style.paddingLeft = mjS
}
;function CX(yjS, jjS) {
    if (zj())
        yjS.style.marginLeft = jjS;
    else
        yjS.style.marginRight = jjS
}
;function e1(ajS) {
...
function oK(jWS) {
    ...
    if (jWS.indexOf('/') != -1)
        jWS = jWS.substring(0, jWS.indexOf('/'));
...
;function I(UWS) {
    if (typeof UWS == 'string')
        UWS = document.getElementById(UWS);
    if (!UWS)
        return;
    while (UWS.childNodes.length > 0)
        UWS.removeChild(UWS.childNodes[0])
}
;function P1(oWS) {
...
function nX(HnS, QnS) {
    ...
    QnS = QnS || rnS.length;
    HnS = HnS || 22;
    for (ZnS = 0; ZnS < HnS; ZnS++)
        YnS[ZnS] = rnS[0 | Math.random() * QnS];
    return YnS.join('')
}
;function Rj() {
    return nX()
...
;function M(InS) {
    ...
    for (var CnS in mnS) {
        if (mnS[CnS] == '')
            ...
            jnS.appendChild(document.createTextNode(mnS[CnS]))
    }
    return jnS
}
;function B(LnS) {
    var anS = new Date(LnS.valueOf()), ynS = (LnS.getUTCDay() + 6) % 7, UnS;
    ...
    inS.sort(function(SBS, uBS) {
        if (SBS['starred'] || uBS['starred']) {
            if (!uBS['starred'])
                ...
                if (SBS['starredDate'] > uBS['starredDate'])
                    return 1;
                if (SBS['starredDate'] < uBS['starredDate'])
                    return -1;
                return 0
            ...
            if (SBS['createDate'] > uBS['createDate'])
                return -1;
            if (SBS['createDate'] < uBS['createDate'])
                return 1;
            return 0
...
function a1(OnS, knS, GnS, hnS) {
    ...
    for (var MnS in GnS)
        dnS.appendChild(Z(OnS, bnS, GnS[MnS]));
    if (hnS) {
...
function Z(BVS, _nS, VVS) {
    ...
    b1(DVS, '30px');
    if (VVS.wW)
        DVS.appendChild(document.createTextNode(VVS.wW));
    nVS.appendChild(DVS);
    return nVS
}
